...
uses
  ...
  FMX.ScrollBox, FMX.Memo, FMX.Clipboard, System.Rtti, eultaskpn,
  FMX.Platform, FMX.Surfaces, fogmath, PythonEngine, FMX.PythonGUIInputOutput;
...
var
  TaskChoise, Param1, Param2, resulting: int64;
  Clpbrd: IFMXExtendedClipboardService;
  Value: Tvalue;
begin
  TimeStart := DateTimeToTimeStamp(Time);
  TaskChoise := TaskNumberComboBox.Items[TaskNumberComboBox.ItemIndex]
    .ToInteger;
  if Param1Edit.Visible then
    Param1 := Param1Edit.Text.ToInt64;
  if Param2Edit.Visible then
    Param2 := Param2Edit.Text.ToInt64;
  ...
  if TaskChoise = 154 then
    resulting:= pascalpyramid;
  if TaskChoise = 155 then
    resulting:= difcondcount(Param1);
  if TaskChoise = 156 then
    resulting:= digsameqnum;
  if TaskChoise = 157 then
    resulting:= fractions10powers(Param1);
  if TaskChoise = 158 then
    resulting:= onelexorderexact;
  if TaskChoise = 159 then
    resulting:= digitrootsumfactors(Param1);
  if TPlatformServices.Current.SupportsPlatformService
    (IFMXExtendedClipboardService, Clpbrd) then
  ...
  TimeMainLabel.Text := ((TimeEnd.Time div 1000).ToString + '.' +
    (TimeEnd.Time mod 1000).ToString + ' s.');
...
begin
  ...
  begin
    TaskHlpr := Tasks[i];
    if TaskHlpr.TaskNum = TaskChoise then
      break
  end;
  ...
  Param1Label.Text := parnam(TaskChoise, TaskHlpr.ParamAmount, 1);
  // available name of possible parameter
  Param1Edit.Text := setdefault(TaskChoise, TaskHlpr.ParamAmount, 1);
  // assign default parameter
  HintLabel.Text := sethint(TaskChoise); // assign hint to show
  if TaskHlpr.ParamAmount > 1 then
  begin
    Param2Label.Text := parnam(TaskChoise, TaskHlpr.ParamAmount, 2);
    // available name of possible parameter
    Param2Edit.Text := setdefault(TaskChoise, TaskHlpr.ParamAmount, 2);
    // assign default parameter
  end;
  If HintLabel.Text <> 'No parameter.' then
