...
type
  ...
  function CheckPalindrNum(number: int64): boolean;
  ...
  function exprvalue(action: string; order, num1, num2, num3,
    num4: int64): int64;
  ...
  procedure checksudoku(var m: Puzzle; var lowestx, lowesty: int64;
    var changed, allcorrect: boolean);
  procedure checksinglefieldsudoku(m: Puzzle; x, y: int64;
    var used: TList<int64>);
  ...
  function checktwoontwo(a1, a2, a3, a4, a5, a6, a7: int64): boolean;
  // helper function for task 103
  function checkthreeonthree(a1, a2, a3, a4, a5, a6, a7: int64): boolean;
  // helper function for task 103
  function ispandigital(a: int64): boolean;
  // checks if the 9-digit number is pandigital
  function isspecial(qset: TList<int64>): boolean;
  // helper function for task 105
  ...
  // greatest common divisor (Euclidus algorythm)
  ...
  // helper for miller-rabin pseudoprimarily test
  function isprimemr(number: int64): boolean;
  // miller-rabin pseudoprimarily test
...
begin
  ...
  begin
    largest := 2002000;
    showmessage
      ('Parameter exceed maximum value: max prime in list is less than 2010000, returning whole existing list')
  end;
  assign(f, 'primes.val');
  reset(f);
  i := 0;
...
begin
  ...
  begin
    largest := 2002000;
    showmessage
      ('Parameter exceed maximum value: max prime in list is less than 2002000, returning whole existing list')
  end;
  assign(f, 'primes.val');
  reset(f);
  i := 0;
...
begin
  ...
  if straight = reverse then
    result := True;
end;

function CheckPalindrStr(txt: string): boolean;
// check if the string is strict palindrome
var
  reverse: string;
  i: integer;
begin
  ...
  for i := 0 to length(txt) - 1 do
    reverse := reverse + txt[length(txt) - i];
  if txt = reverse then
    result := True;
end;
...
function DigToChr(number: int64): string;
// convert digit into spaceand symbol string
begin
  if number = 1 then
    result := ' 1';
  if number = 2 then
...
function RightStr(txt: string; amount: integer): string;
// returns amounting symbols from the right of the string
var
  i: integer;
begin
  ...
  begin
    showmessage
      ('Requested amount of returning symbols exceeds the length of source string. Returning source string.');
    result := txt;
  end
  else
  begin
    result := '';
    if amount <> 0 then
      for i := length(txt) - amount + 1 to length(txt) do
        result := result + txt[i];
  end;
end;

function LeftStr(txt: string; amount: integer): string;
// returns amounting symbols from the left of the string
var
  i: integer;
begin
  if amount > length(txt) then
...
begin
  ...
  if intmd < 20 then
    case intmd of
      ...
      2:
        result := result + ' and ' + 'two';
      3:
        result := result + ' and ' + 'three';
      4:
        result := result + ' and ' + 'four';
      5:
        result := result + ' and ' + 'five';
      6:
        result := result + ' and ' + 'six';
      7:
        result := result + ' and ' + 'seven';
      8:
        result := result + ' and ' + 'eight';
      9:
        result := result + ' and ' + 'nine';
      10:
        result := result + ' and ' + 'ten';
      11:
        result := result + ' and ' + 'eleven';
      12:
        result := result + ' and ' + 'twelve';
      13:
        result := result + ' and ' + 'thirteen';
      14:
        result := result + ' and ' + 'fourteen';
      15:
        result := result + ' and ' + 'fifteen';
      16:
        result := result + ' and ' + 'sixteen';
      17:
        result := result + ' and ' + 'seventeen';
      18:
        result := result + ' and ' + 'eighteen';
      19:
        result := result + ' and ' + 'nineteen';
    end;
  if intmd >= 20 then
  begin
    case intmd div 10 of
      2:
        result := result + ' and twenty';
      3:
        result := result + ' and thirty';
      4:
        result := result + ' and forty';
      5:
        result := result + ' and fifty';
      6:
        result := result + ' and sixty';
      7:
        result := result + ' and seventy';
      8:
        result := result + ' and eighty';
      9:
        result := result + ' and ninety';
    end;
    if intmd mod 10 > 0 then
      result := result + '-';
...
function DeleteSpcAndDef(txt: string): string;
// deletes spaces and defises from string
var
  i: integer;
begin
  result := '';
  for i := 1 to length(txt) do
    if (txt[i] <> ' ') and (txt[i] <> '-') then
      result := result + txt[i];
end;

function strmultdig(inner: string; mlt: integer): string;
// multiply string of digits to one-digit number and return string
var
  i, ovfl, res: int64;
begin
  result := '';
  ...
  if ovfl > 0 then
    result := DigToChr(ovfl)[2] + result;
end;

function sumstrings(num1, num2: string): string;
// return string which is equal to integer sum of two "integer-strings"
var
  i, ovfl: integer;
  l, m: string;
  nhlp: string;
begin
  ...
  begin
    result := DigToChr((StrToInt(l[i]) + StrToInt(m[length(m) - (length(l) - i)]
      ) + ovfl) mod 10)[2] + result;
    ovfl := (StrToInt(l[i]) + StrToInt(m[length(m) - length(l) + i]) +
      ovfl) div 10;
  end;
  nhlp := '';
...
function powerint64(number1, number2: int64): int64;
// returns number1 in number2-th power in int64
var
  i: int64;
begin
  result := 1;
...
function dectobin(number: int64): string;
// converts number to string in binary mode
begin
  result := '';
  if number = 0 then
    result := '0';
...
function pokercomb(cards: string): int64;
// returns fully comparable numeric description for poker combinations
var
  i: int64;
  fn, fs: TList<int64>;
  strt, flus: boolean;
begin
  ...
  fs.Sort;
  result := 0;
  strt := (fn[4] - fn[3] = 1) and (fn[3] - fn[2] = 1) and (fn[2] - fn[1] = 1)
    and (fn[1] - fn[0] = 1);
  flus := (fs[4] = fs[3]) and (fs[3] = fs[2]) and (fs[2] = fs[1]) and
    (fs[1] = fs[0]);
  if ((fn[0] = fn[1]) and (fn[1] = fn[2]) and (fn[2] = fn[3])) or
    ((fn[4] = fn[3]) and (fn[3] = fn[2]) and (fn[2] = fn[1])) then
    result := 70000000000; // four of a kind
  if ((fn[0] = fn[1]) and (fn[2] = fn[3]) and (fn[3] = fn[4])) then
    result := 60000000000 + fn[2]; // full house 2+3
  if ((fn[0] = fn[1]) and (fn[1] = fn[2]) and (fn[3] = fn[4])) then
    result := 60000000000 + fn[0]; // full house 3+2
  if strt and flus then
    result := 80000000000 + fn[0]; // straight flush
  if (result = 8) and (fn[0] = 10) then
    result := 90000000000; // royal flush
  if (not strt) and flus then
    result := 50000000000 + fn[4] * 100000000 + fn[3] * 1000000 + fn[2] * 10000
      + fn[1] * 100 + fn[0]; // flush
  if strt and (not flus) then
    result := 40000000000 + fn[0]; // straight
  if ((fn[0] = fn[1]) and (fn[1] = fn[2]) and (fn[2] <> fn[3]) and
    (fn[3] <> fn[4])) or ((fn[0] <> fn[1]) and (fn[1] = fn[2]) and
    (fn[2] = fn[3]) and (fn[3] <> fn[4])) or
    ((fn[0] <> fn[1]) and (fn[1] <> fn[2]) and (fn[2] = fn[3]) and
    (fn[3] = fn[4])) then
    result := 30000000000; // three of a kind
  if (fn[0] = fn[1]) and (fn[1] <> fn[2]) and (fn[2] <> fn[3]) and
    (fn[3] = fn[4]) then
    result := 20000000000 + fn[3] * 10000 + fn[0] * 100 + fn[2];
  // two pairs  2+1+2
  if (fn[0] <> fn[1]) and (fn[1] = fn[2]) and (fn[2] <> fn[3]) and
    (fn[3] = fn[4]) then
    result := 20000000000 + fn[3] * 10000 + fn[1] * 100 + fn[0];
  // two pairs  1+2+2
  if (fn[0] = fn[1]) and (fn[1] <> fn[2]) and (fn[2] = fn[3]) and
    (fn[3] <> fn[4]) then
    result := 20000000000 + fn[2] * 10000 + fn[0] * 100 + fn[4];
  // two pairs  2+2+1
  if (fn[0] = fn[1]) and (fn[1] <> fn[2]) and (fn[2] <> fn[3]) and
    (fn[3] <> fn[4]) then
    result := 10000000000 + fn[0] * 1000000 + fn[4] * 10000 + fn[3] * 100 +
      fn[2]; // one pair  2+1+1+1
  if (fn[0] <> fn[1]) and (fn[1] = fn[2]) and (fn[2] <> fn[3]) and
    (fn[3] <> fn[4]) then
    result := 10000000000 + fn[1] * 1000000 + fn[4] * 10000 + fn[3] * 100 +
      fn[0]; // one pair  1+2+1+1
  if (fn[0] <> fn[1]) and (fn[1] <> fn[2]) and (fn[2] = fn[3]) and
    (fn[3] <> fn[4]) then
    result := 10000000000 + fn[2] * 1000000 + fn[4] * 10000 + fn[1] * 100 +
      fn[0]; // one pair  1+1+2+1
  if (fn[0] <> fn[1]) and (fn[1] <> fn[2]) and (fn[2] <> fn[3]) and
    (fn[3] = fn[4]) then
    result := 10000000000 + fn[3] * 1000000 + fn[2] * 10000 + fn[1] * 100 +
      fn[0]; // one pair  1+1+1+2
  if result = 0 then
    result := 100000000 * fn[4] + 1000000 * fn[3] + 10000 * fn[2] + 100 * fn[1]
      + fn[0]; // no combination
end;
...
function codetoascii(i: integer): char;
// returns symbol for ascii code in integer
begin
  case i of
    32:
      result := ' ';
...
function IsHeptagonal(number: int64): boolean;
// check if the number is heptagonal
begin
  result := (trunc((sqrt(40 * number + 9) + 3) / 10))
    = (sqrt(40 * number + 9) + 3) / 10;
end;

function IsOctagonal(number: int64): boolean;
// check if the number is octagonal
begin
  result := (trunc((sqrt(3 * number + 1) + 1) / 3))
    = (sqrt(3 * number + 1) + 1) / 3;
end;

function NextOctagonal(number: int64): int64;
// return next closest higher number that is octagonal
var
  i: int64;
begin
...
begin
  result := (trunc((sqrt(8 * number + 1) - 1) / 2))
    = (sqrt(8 * number + 1) - 1) / 2;
end;
...
function stringmult(str1, str2: string): string;
// returns the product of two numeric strings in string format
var
  interm: TList<string>;
  res: string;
  i, j, k, ovfl: int64;
...
function eulerfunction(number: int64): int64;
// returns the value of euler (fi) function
var
  i, factor: int64;
  multies: TList<int64>;
begin
  ...
  begin
    ...
    begin
      if multies[i] = multies[i - 1] then
        result := result * multies[i]
      else
...
function hghcomfac(number1, number2: int64): int64;
// returns highest common factor of a couple numbers
var
  i, j, k: int64;
begin
  j := number1;
...
function subtrstrings(str1, str2: string): string;
// substract second integer in string record from first
var
  i: int64;
...
begin
  ...
  result := '-';
  if (length(str2) > length(str1)) or
    ((length(str1) = length(str2)) and (str2 > str1)) then
    result := '';
  if str1 = '0' then
    result := '';
  if (length(str1) = length(str2)) and (str2 = str1) then
    result := '0';
  if result = '-' then
  begin
    result := '';
...
begin
  ...
  begin
    ...
    step := step + 1;
    if step <= length(str1) then
      subtr := subtr + str1[step];
    if (length(subtr) > 1) and (subtr[1] = '0') then
      subtr := RightStr(subtr, length(subtr) - 1)
  end;
end;

function stringintdivision(str1, str2: string; precision: int64): string;
// divides first dtring record of a number into second taking certain digits in result
var
  i, mult, precis: int64;
  digit: integer;
  subtr: string;
...
function factorization(number: int64): TList<int64>;
// returns the list of all prime factors of a number
var
  i, numb: int64;
begin
...
function RomanToInt64(rnumber: string): int64;
// converts correct roman record of number into decimal
var
  i: integer;
begin
  i := 1;
  ...
  begin
    if ((rnumber[i] = 'I') and ((rnumber[i + 1] = 'V') or (rnumber[i + 1] = 'X')
      )) or ((rnumber[i] = 'X') and ((rnumber[i + 1] = 'L') or
      (rnumber[i + 1] = 'C'))) or
      ((rnumber[i] = 'C') and ((rnumber[i + 1] = 'D') or (rnumber[i + 1] = 'M')))
    then
    begin
      if (rnumber[i] = 'I') and
        ((rnumber[i + 1] = 'V') or (rnumber[i + 1] = 'X')) then
        if rnumber[i + 1] = 'V' then
          result := result + 4
        else
          result := result + 9;
      if (rnumber[i] = 'X') and
        ((rnumber[i + 1] = 'L') or (rnumber[i + 1] = 'C')) then
        if rnumber[i + 1] = 'L' then
          result := result + 40
        else
          result := result + 90;
      if (rnumber[i] = 'C') and
        ((rnumber[i + 1] = 'D') or (rnumber[i + 1] = 'M')) then
        if rnumber[i + 1] = 'D' then
          result := result + 400
        else
          result := result + 900;
      i := i + 1;
...
function Int64ToMinRoman(number: int64): string;
// converts integer up to 1000000 into roman minimal record
var
  intmd: int64;
begin
...
// helper to task 93
var
  intmd1, intmd2, n1, n2, n3, n4: extended;
begin
  n1 := extended(num1);
  n2 := extended(num2);
  n3 := extended(num3);
...
procedure readsudoku(fname: string; number: int64; var m: Puzzle);
// reads single sudoku from file, helper for task 96
var
  f: text;
  i, j: int64;
  row: string;
...
procedure checksudoku(var m: Puzzle; var lowestx, lowesty: int64;
  var changed, allcorrect: boolean);
// checks the whole single puzzle, helper for task 96
var
  i, j, j1, im, jm, lowestamnt: int64;
  used: TList<int64>;
begin
...
begin
  ...
  for i := 1 to 3 do
    for j := 1 to 3 do
      if not((im * 3 + i = x) and (jm * 3 + j = y)) and
        (m.a[im * 3 + i, jm * 3 + j] <> 0) and
        (not(used.Contains(m.a[im * 3 + i, jm * 3 + j]))) then
        used.Add(m.a[im * 3 + i, jm * 3 + j]);
end;
...
begin
  ...
  if matrix.Count = 0 then
    rslt := 0;
  if matrix.Count = 1 then
    rslt := matrix[0];
  if matrix.Count = 4 then
    rslt := matrix[0] * matrix[3] - matrix[1] * matrix[2];
  if matrix.Count > 4 then
  begin
    for i := 0 to length(a) - 1 do
      for j := 0 to length(a) - 1 do
...
begin
  ...
  a[6] := a6;
  a[7] := a7;
  if a[1] + a[2] + a[4] = a[3] + a[5] + a[6] then
    result := False;
  if a[1] + a[2] + a[4] = a[3] + a[5] + a[7] then
    result := False;
  if a[1] + a[2] + a[4] = a[3] + a[6] + a[7] then
    result := False;
  if a[1] + a[2] + a[5] = a[3] + a[4] + a[6] then
    result := False;
  if a[1] + a[2] + a[5] = a[3] + a[4] + a[7] then
    result := False;
  if a[1] + a[2] + a[5] = a[3] + a[6] + a[7] then
    result := False;
  if a[1] + a[2] + a[6] = a[3] + a[4] + a[5] then
    result := False;
  if a[1] + a[2] + a[6] = a[3] + a[4] + a[7] then
    result := False;
  if a[1] + a[2] + a[6] = a[3] + a[5] + a[7] then
    result := False;
  if a[1] + a[2] + a[7] = a[3] + a[4] + a[5] then
    result := False;
  if a[1] + a[2] + a[7] = a[3] + a[4] + a[6] then
    result := False;
  if a[1] + a[2] + a[7] = a[3] + a[5] + a[6] then
    result := False;
  if a[1] + a[3] + a[4] = a[2] + a[5] + a[6] then
    result := False;
  if a[1] + a[3] + a[4] = a[2] + a[5] + a[7] then
    result := False;
  if a[1] + a[3] + a[4] = a[2] + a[6] + a[7] then
    result := False;
  if a[1] + a[3] + a[5] = a[2] + a[4] + a[6] then
    result := False;
  if a[1] + a[3] + a[5] = a[2] + a[4] + a[7] then
    result := False;
  if a[1] + a[3] + a[5] = a[2] + a[6] + a[7] then
    result := False;
  if a[1] + a[3] + a[6] = a[2] + a[4] + a[5] then
    result := False;
  if a[1] + a[3] + a[6] = a[2] + a[4] + a[7] then
    result := False;
  if a[1] + a[3] + a[6] = a[2] + a[5] + a[7] then
    result := False;
  if a[1] + a[3] + a[7] = a[2] + a[4] + a[5] then
    result := False;
  if a[1] + a[3] + a[7] = a[2] + a[4] + a[6] then
    result := False;
  if a[1] + a[3] + a[7] = a[2] + a[5] + a[6] then
    result := False;
  if a[1] + a[4] + a[5] = a[2] + a[3] + a[6] then
    result := False;
  if a[1] + a[4] + a[5] = a[2] + a[3] + a[7] then
    result := False;
  if a[1] + a[4] + a[5] = a[2] + a[6] + a[7] then
    result := False;
  if a[1] + a[4] + a[6] = a[2] + a[3] + a[5] then
    result := False;
  if a[1] + a[4] + a[6] = a[2] + a[3] + a[7] then
    result := False;
  if a[1] + a[4] + a[6] = a[2] + a[5] + a[7] then
    result := False;
  if a[1] + a[4] + a[7] = a[2] + a[3] + a[5] then
    result := False;
  if a[1] + a[4] + a[7] = a[2] + a[3] + a[6] then
    result := False;
  if a[1] + a[4] + a[7] = a[2] + a[5] + a[6] then
    result := False;
  if a[1] + a[5] + a[6] = a[2] + a[3] + a[4] then
    result := False;
  if a[1] + a[5] + a[6] = a[2] + a[3] + a[7] then
    result := False;
  if a[1] + a[5] + a[6] = a[2] + a[4] + a[7] then
    result := False;
  if a[1] + a[5] + a[7] = a[2] + a[3] + a[4] then
    result := False;
  if a[1] + a[5] + a[7] = a[2] + a[3] + a[6] then
    result := False;
  if a[1] + a[5] + a[7] = a[2] + a[4] + a[6] then
    result := False;
  if a[1] + a[6] + a[7] = a[2] + a[3] + a[4] then
    result := False;
  if a[1] + a[6] + a[7] = a[2] + a[3] + a[5] then
    result := False;
  if a[1] + a[6] + a[7] = a[2] + a[4] + a[5] then
    result := False;
end;
...
begin
  ...
  begin
    ...
    if not((numberstr[1] = '1') and (numberstr[2] = '2') and
      (numberstr[3] = '3') and (numberstr[4] = '4') and (numberstr[5] = '5') and
      (numberstr[6] = '6') and (numberstr[7] = '7') and (numberstr[8] = '8') and
      (numberstr[9] = '9')) then
      result := False;
  end;
end;
...
begin
  ...
  if qset[qset.Count - 1] >= qset[0] + qset[1] then
    result := False;
  if result then
    if qset[qset.Count - 1] + qset[qset.Count - 2] >= qset[0] + qset[1] + qset[2]
    then
      result := False;
  if result then
    if qset[qset.Count - 1] + qset[qset.Count - 2] + qset[qset.Count - 3] >=
      qset[0] + qset[1] + qset[2] + qset[3] then
      result := False;
  if result then
  ...
  if qset.Count > 8 then
    if result then
      if qset[qset.Count - 1] + qset[qset.Count - 2] + qset[qset.Count - 3] +
        qset[qset.Count - 4] >= qset[0] + qset[1] + qset[2] + qset[3] + qset[4]
      then
        result := False;
  ...
  if qset.Count > 10 then
    if result then
      if qset[qset.Count - 1] + qset[qset.Count - 2] + qset[qset.Count - 3] +
        qset[qset.Count - 4] + qset[qset.Count - 5] >= qset[0] + qset[1] +
        qset[2] + qset[3] + qset[4] + qset[5] then
        result := False;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if sums.Contains(qset[i] + qset[j] + qset[k] + qset[l] + qset[m])
            then
            begin
              result := False;
              break
...
begin
  result := False;
  if reversenumber(number) = number then
    result := True;
end;
...
function isprimemr(number: int64): boolean;
// probabilistic miller-rabin check if the number is prime
var
  i: int64;
begin
  result := True;
  if (number <= 1) or (number mod 2 = 0) or (number mod 3 = 0) or
    (number mod 5 = 0) or (number mod 7 = 0) then
    result := False;
...
begin
  ...
  begin
    result := result + 21 *
      (((powerint64(7, i - 1) - 1 + 1) * ((powerint64(7, i - 1)) - 1)) div 2) *
